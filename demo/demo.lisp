...
(defun start ()
  (let ((game (make-instance 's2e:engine :width *window-width* :height *window-height*
    ...
    (s2e:add-keyboard-mapping game "left" :left 'move-left)
    (s2e:add-keyboard-mapping game "right" :right 'move-right)
    (s2e:add-keyboard-mapping game "up" :up 'move-up)
    (s2e:add-keyboard-mapping game "down" :down 'move-down)
    (s2e:add-keyboard-mapping game "space" :space 'display-bouding-boxes)
